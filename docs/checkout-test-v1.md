...
5. Définir `CRON_SECRET`, puis programmer un appel GET à `/api/cron/checkouts` avec `Authorization: Bearer <secret>` via le planificateur de l’hébergement, par exemple toutes les cinq minutes. Aucune tâche distante n’a été créée. Surveiller les compteurs `failed` et `needsReview` ; ils ne déclenchent pas d’email.
