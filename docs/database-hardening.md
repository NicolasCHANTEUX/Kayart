...
Le stockage local est réservé au développement. Sur l’hébergement, configurer Supabase. N’activer `KAYART_ENABLE_MANUAL_ORDERS=true` que si l’atelier enregistre volontairement des ventes prises hors Stripe (espèces, virement, retrait) ; cette fonctionnalité est volontairement indépendante du stock et de la disponibilité du catalogue. Un secret HMAC dédié `PRODUCT_IMAGE_RECEIPT_SECRET` est possible ; à défaut, la clé Supabase du serveur sert à signer les reçus.
