...
const adminLinks = [["/admin", "Tableau de bord"], ["/admin/produits", "Produits"], ["/admin/commandes", "Commandes"], ["/admin/demandes", "Demandes clients"], ["/admin/livraison", "Livraison & réglages"]];

export function AdminQuickAccess() {
  const pathname = usePathname();
  if (pathname === "/admin" || pathname.startsWith("/admin/")) return null;
  return <div className="admin-quick-access"><div className="container admin-quick-access__inner">
    <span className="admin-quick-access__label">Administration</span>
    <nav aria-label="Accès à l’administration">{adminLinks.map(([href, label]) => <Link key={href} href={href}>{label}<span aria-hidden="true">↗</span></Link>)}</nav>
  </div></div>;
}
...
export function AdminNavigation() {
  const pathname = usePathname();
  return <nav className="admin-navigation" aria-label="Administration"><span className="admin-navigation__label">Administration</span>
    {adminLinks.map(([href, label]) => <Link key={href} href={href} aria-current={pathname === href || (href !== "/admin" && pathname.startsWith(href + "/")) ? "page" : undefined}>{label}<span aria-hidden="true">↗</span></Link>)}
