...
import { mapPrismaAdminOrder, mapPrismaProduct, mapPublicPrismaProduct } from "./catalog.mapper";
...
export async function searchShop(params: ListParams) {
  ...
  if (process.env.KAYART_DATA_SOURCE !== "prisma") {
    ...
    const rows = all.filter(p => (!q || [p.name,p.sku,p.description].some(v => v.toLowerCase().includes(q.toLowerCase()))) && (!category || p.categoryId === selected?.id) && (!condition || p.condition === condition) && (!stock || (p.availability === "available" && (p.stockQuantity ?? 0) > 0)));
    rows.sort((a,b) => {
  ...
  const where: Prisma.ProductWhereInput = { ...visible,
    ...(q ? { OR: ["name", "sku", "description"].map(field => ({ [field]: { contains: q, mode: "insensitive" } })) } : {}),
