...
export default async function RequestsPage({ searchParams }: { searchParams?: Promise<{ type?: string; page?: string; status?: string; error?: string; updated?: string }> }) {
  ...
  return <section className="section admin-page"><div className="container">
    ...
    <div className="request-list">{requests.map(request => <article className="feature-card" key={request.id}>
      ...
      <form action={updateRequestStatusAction} className="request-filter"><input type="hidden" name="kind" value={kind} /><input type="hidden" name="id" value={request.id} /><input type="hidden" name="updatedAt" value={request.updatedAt} /><input type="hidden" name="previousStatus" value={request.status} /><input type="hidden" name="page" value={page} /><input type="hidden" name="filterStatus" value={status ?? ""} /><label>Statut<select name="status" defaultValue={request.status}>{requestStatuses.map(value => <option key={value} value={value}>{requestStatusLabels[value]}</option>)}</select></label><button className="button button--primary">Enregistrer</button></form>
    </article>)}</div>
