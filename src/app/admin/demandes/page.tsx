...
export default async function RequestsPage({ searchParams }: { searchParams?: Promise<{ type?: string; page?: string; status?: string; trash?: string }> }) {
  ...
  return <section className="section admin-page"><div className="container">
    ...
    <nav className="header-actions" aria-label="Type de demande">{requestKinds.map(type => <Link aria-current={type === kind ? "page" : undefined} className="button button--ghost" key={type} href={url(type, 1, undefined)}>{requestKindLabels[type]}</Link>)}</nav>
    <nav className="header-actions" aria-label="Affichage des demandes"><Link aria-current={!trashed ? "page" : undefined} href={url(kind, 1, status, false)}>Demandes</Link><Link aria-current={trashed ? "page" : undefined} href={url(kind, 1, status, true)}>Corbeille</Link></nav>
